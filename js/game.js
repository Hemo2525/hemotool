...
function initGame() {
    ...
    let lockItemArea = document.querySelector('[class^=___program-neta-button___] [class^=___inner-content___]');
    if(lockItemArea){
        ...
        CheckAndWipe(document.querySelector('[class^=___program-neta-button___] .image'));

    } else {
        console.error("起動中アイテムのエリアを監視に失敗");
    }
}
...
function CheckAndWipe(launchItem) {
    if(launchItem) {
        let gameName = launchItem.getAttribute('alt');
        console.log(gameName + " をワイプにします");
        ...
        console.error("起動中ゲームのエリアのDOMがありません", launchItem);
    }
}
    ...
    for (const mutation of mutationRecords) {
        ...
        if(mutation.addedNodes){
            ...
            mutation.addedNodes.forEach((currentNode) => {
                ...
                //console.log("追加ノードがあります", currentNode);
                CheckAndWipe(currentNode);
            });
        }
        ...
        if(mutation.removedNodes){
            ...
            mutation.removedNodes.forEach((currentNode) => {
                ...
                console.log("削除ノードがあります", currentNode);
                document.querySelector('body').removeAttribute('ext-master-wipe');

                // (querySelectorはDOMが削除されているので使えないのでclassNameで判定)
                /*
                if(currentNode.className.startsWith('___program-neta-button___')){
                    document.querySelector('body').removeAttribute('ext-master-wipe');
                }
                */
            });
